...
def create_q_model():
    ...
    hand_layer1 = layers.Conv1D(16,4,strides=4, name="hand_layer1", activation="relu")
    hand_layer2 = layers.Dense(64, name="hand_layer2", activation="relu")

    hist_input = layers.Input(shape=(hist_len,num_cards,1,), name="history")
    hist_inter1 = hand_layer2(hand_layer1(hist_input))
    hist_inter2 = layers.Conv2D(32,4, strides=4, activation="relu")(hist_inter1)
    hist_output = layers.Flatten()(hist_inter2)

    current_hand_input = layers.Input((num_cards,1,), name="hand")
    current_hand_input2 = layers.Conv1D(16,4,4, activation="relu")(current_hand_input)
    current_hand_output = layers.Flatten()(current_hand_input2)
    current_hand_output = layers.Dense(64, activation="relu")(current_hand_output)

    move_input = layers.Input((num_cards,1,), name="move")
    move_input2 = layers.Conv1D(16,4,4, activation="relu")(move_input)
    move_output = layers.Flatten()(move_input2)
    move_output = layers.Dense(128, activation="relu")(move_output)
